...
func (vc *VariableConfig) ReplaceTextTemplate(path string) error {
	...
	for fileScanner.Scan() {
		...
		for {
			...
			if present {
				deprecationWarning := fmt.Sprintf("Detected usage of a deprecated template: '%s' changed to '%s'.  Please notify your package creator for an update.", templateKey, vc.deprecatedKeys[templateKey])
